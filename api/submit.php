...
$all = 'Kagamine Yes!' . strval($contact) . $confessor . $ta . $image . $introduction . $content . $class . strval($timestamp);
...
try {
    $pdo = pdoConnect();
    $stmt = $pdo->prepare("insert into loveway_data(id,favorite,confessor,contact,class,time,to_who,introduction,content,image,comment)values(?,?,?,?,?,?,?,?,?,?,?)");
    $love_id = rand(100000000, 999999999);
    $stmt->bindValue(1, $love_id);
    $stmt->bindValue(2, 0);
    $stmt->bindValue(3, $confessor);
    $stmt->bindValue(4, $contact);
    $stmt->bindValue(5, $class);
    $stmt->bindValue(6, date("Y-m-d H:i:s", time()));
    $stmt->bindValue(7, $ta);
    $stmt->bindValue(8, $introduction);
    $stmt->bindValue(9, $content);
    $stmt->bindValue(10, $image);
    $stmt->bindValue(11, "[]");
    if ($stmt->execute()) {
